...
(function() {
  ...
  const _notModeMap = {
    fr: { 'WAR': 'Hors guerre', 'RAID': 'Hors raid', 'CRUCIBLE SHOWDOWN': 'Hors confrontation du Creuset' },
    de: { 'WAR': 'Nicht im KRIEG', 'RAID': 'Nicht in RAUBZÜGEN', 'CRUCIBLE SHOWDOWN': 'Nicht bei SCHMELZTIEGEL-SHOWDOWN' },
    es: { 'WAR': 'Fuera de guerras', 'RAID': 'Fuera de las incursiones', 'CRUCIBLE SHOWDOWN': 'Fuera del enfrentamiento del Crisol' },
    pt: { 'WAR': 'Fora da GUERRA', 'RAID': 'Fora das INCURSÕES', 'CRUCIBLE SHOWDOWN': 'Fora do CONFRONTO DO CADINHO' },
    it: { 'WAR': 'Non in GUERRA', 'RAID': 'Non negli ASSALTI', 'CRUCIBLE SHOWDOWN': 'Non nello SCONTRO DEL CROGIOLO' },
    ja: { 'WAR': '戦争以外では', 'RAID': 'レイド以外では', 'CRUCIBLE SHOWDOWN': 'クルーシブル対決以外では' },
    ko: { 'WAR': '전쟁 외', 'RAID': '레이드 외', 'CRUCIBLE SHOWDOWN': '크루시블 대결 외' },
    ru: { 'WAR': 'Вне ВОЙНЫ', 'RAID': 'Вне РЕЙДОВ', 'CRUCIBLE SHOWDOWN': 'Вне ГОРНИЛА' },
  };
  ...
  const _P = {
    ...
    // Kahhori
    targetNotTraitDeal:   /^If the primary target is not (.+?), Deal (\d+)% of target's Max Health\.$/,
    targetTraitDeal:      /^If the primary target is (.+?), Deal (\d+)% of target's Max Health\.$/,
    ...
    otherwiseDeal:        /^Otherwise, Deal (\d+)% of target's Max Health\.$/,
    ...
    dealFlat:             /^Deal (\d+)% of target's Max Health\.$/,
    ...
    otherwiseAttackPierce:     /^Otherwise, attack for (\d+)% Piercing instead\.$/,
    otherwiseAttackDmgPierce:  /^Otherwise, attack for (\d+)% damage \+ (\d+)% Piercing instead\.$/,
    otherwiseAttackDmg:        /^Otherwise, attack for (\d+)% damage instead\.$/,
    ...
    selfNotHasGainMax:  /^If self does not have (.+?), Gain \+(\d+) (.+?), up to a maximum of (\d+)\.$/,
    ...
    targetHasGainPlusMax:      /^If the primary target has (.+?), Gain \+(\d+) (.+?), up to a maximum of (\d+)\.$/,
    // Group 5: Mode + trigger combos
    modeOnCritApply:           /^In (.+?), On Crit, Apply \+(\d+) (.+?) to the primary target\.$/,
    modeOnCritApplyProc:       /^In (.+?), On Crit, Apply (.+?) to the primary target\.$/,
    modeOnCritGenEnergyAllAllies: /^In (.+?), On Crit, Generate \+(\d+) Ability Energy for all allies\.$/,
    modeIfAllyApplyProc:       /^In (.+?), If (.+?) is an ally, Apply (.+?) to the primary target\.$/,
    modeIfAllyReduceSpeed:     /^In (.+?), If (.+?) is an ally, Reduce Speed Bar by (\d+)%\.$/,
    modeIfAllyDmg:             /^In (.+?), If (.+?) is an ally, \+(\d+)% damage\.$/,
    modeIfAllyDmgPierce:       /^In (.+?), If (.+?) is an ally, \+(\d+)% damage \+ (\d+)% Piercing\.$/,
    modeAnyEnemyHasFlip:       /^In (.+?), If any enemy has positive effects, Flip (\d+) positive effect\(s\) to negative on the primary target\.$/,
    modeTargetHasApplyPlusRandomAlly: /^In (.+?), If the primary target has (.+?), Apply \+(\d+) (.+?) to a random ally\.$/,
    modeOnCritIfTraitAlliesApplyProc: /^In (.+?), On Crit, If (\d+)\+ (.+?) allies, Apply (.+?) to the primary target\.$/,
    modeSelfHasDmgPierce:      /^In (.+?), If self has (.+?), \+(\d+)% damage \+ (\d+)% Piercing\.$/,
    // --- Note patterns ---
    noteKilledCantRevive:      /^Enemies killed by this attack cannot be revived\.$/,
    noteAttackCantCountered:   /^This attack cannot be countered\.$/,
    noteAttackCantBlocked:     /^This attack cannot be blocked\.$/,
    noteAttackCantDodged:      /^This attack cannot be dodged\.$/,
    noteAttackCantMiss:        /^This attack cannot miss\.$/,
    ...
    onAssistGainPlusMax:    /^On Assist, Gain \+(\d+) (.+?), up to a maximum of (\d+)\.$/,
    ...
    // Chain targeting patterns
    pierceAdjacentOne:         /^\+(\d+)% Piercing to 1 adjacent target\.$/,
    dmgPierceAdjacentOne:      /^\+(\d+)% damage \+ (\d+)% Piercing to 1 adjacent target\.$/,
    dmgAdjacentOne:            /^\+(\d+)% damage to 1 adjacent target\.$/,
    pierceAdjacentN:           /^\+(\d+)% Piercing to up to (\d+) adjacent targets\.$/,
    dmgPierceAdjacentN:        /^\+(\d+)% damage \+ (\d+)% Piercing to up to (\d+) adjacent targets\.$/,
    dmgAdjacentN:              /^\+(\d+)% damage to up to (\d+) adjacent targets\.$/,
    dmgAdjacent:               /^\+(\d+)% damage to adjacent enemies\.$/,
    dmgPierceAdjacent:         /^\+(\d+)% damage \+ (\d+)% Piercing to adjacent enemies\.$/,
    dmgCritChanceAdjacent:     /^\+(\d+)% damage \+ (\d+)% Crit Chance to adjacent enemies\.$/,
    dmgDrainAdjacentN:         /^\+(\d+)% damage \+ (\d+)% Drain to up to (\d+) adjacent targets\.$/,
    // Chain note patterns
    noteChainBrokenCounter:    /^Chain is broken by counter-attack\.$/,
    noteChainBrokenDodge:      /^Chain is broken by dodge\.$/,
    // Mode/conditional + chain compound patterns
    modeDmgAdjacentOne:        /^In (.+?), \+(\d+)% damage to 1 adjacent target\.$/,
    modeDmgAdjacentN:          /^In (.+?), \+(\d+)% damage to up to (\d+) adjacent targets\.$/,
    notModeDmgAdjacentOne:     /^Not in (.+?), \+(\d+)% damage to 1 adjacent target\.$/,
    selfHasMoreDmgAdjacentOne: /^If self has more than (\d+) (.+?), \+(\d+)% damage to 1 adjacent target\.$/,
    targetHasAnyNegDmgAdjacentN: /^If the primary target has any negative effects, \+(\d+)% damage to up to (\d+) adjacent targets\.$/,
    targetHasAnyNegCopyNegExcl: /^If the primary target has any negative effects, Copy (\d+) negative effect\(s\) from the primary target, excluding (.+?)\.$/,
    bareDmgBonus:              /^\+(\d+)% damage\.$/,
    modeApplyPlusRandomAlly:   /^In (.+?), Apply \+(\d+) (.+?) to a random ally\.$/,
    dealAlliesHealth:         /^Deal (\d+)% of allies' Max Health\.$/,
    ...
    targetHasDealRedistribute:/^If the primary target has (.+?), Deal (\d+)% of target's Max Health and redistribute to (.+?)\.$/,
    ...
    selfHasLessThanGainMax: /^If self has less than (\d+) (.+?), Gain \+(\d+) (.+?), up to a maximum of (\d+)\.$/,
    ...
    modeSubTargetHasDeal:     /^In (.+?), (.+?), If the primary target has (.+?), Deal (\d+)% of target's Max Health\.$/,
    ...
    modeSubTargetNotTraitDeal:/^In (.+?), (.+?), If the primary target is not (.+?), Deal (\d+)% of target's Max Health\.$/,
    ...
    selfHasLoseProc:           /^If self has (.+?), Lose (\d+) (.+?)\.$/,
    selfHasMoreSelfHasLoseProc:/^If self has more than (\d+) (.+?), If self has (.+?), Lose (\d+) (.+?)\.$/,
    ...
    ifAllyApply:               /^If (.+?) is an ally, Apply (.+?) to the primary target\.$/,
    ...
    otherwiseTargetHasGainPlus: /^Otherwise, If the primary target has (.+?), Gain \+(\d+) (.+?)\.$/,
    ...
    modeApplyProcAllies:       /^In (.+?), Apply (.+?) to allies\.$/,
    ...
    modeSubTargetHasRemoveTarget: /^In (.+?, .+?), If the primary target has (.+?), Remove (.+?) from the primary target\.$/,
    notModeCountTarget:        /^Not in (.+?), Clear (\d+) positive effect\(s\) from the primary target\.$/,
    modeSubNoteEnemiesKilled:  /^In (.+?, .+?), enemies killed by this attack cannot be revived\.$/,
    selfHasAnyNegStealAllNeg:  /^If self has any negative effects, Steal all negative effect\(s\) from the primary target\.$/,
    otherwiseStealAllExcl:     /^Otherwise, Steal all positive effect\(s\) from the primary target, excluding (.+?)\.$/,
    onAssistStealAllExcl:      /^On Assist, Steal all positive effect\(s\) from the primary target, excluding (.+?)\.$/,
    selfNoNegNotModeFlipTarget:/^If self has no negative effects, Not in (.+?), Flip (\d+) positive effect\(s\) to negative on the primary target\.$/,
    otherwiseNotModeFlipAllies:/^Otherwise, Not in (.+?), Flip (\d+) negative effect\(s\) to positive on allies\.$/,
    modeFlipNegAllies:         /^In (.+?), Flip (\d+) negative effect\(s\) to positive on allies\.$/,
    anyEnemyHasFlipTarget:     /^If any enemy has positive effects, Flip (\d+) positive effect\(s\) to negative on the primary target\.$/,
    copyCountExclTwo:          /^Copy (\d+) positive effect\(s\) from the primary target, excluding (.+?) and (.+?)\.$/,
    selfHasProcClearAllNegAllies: /^If self has (.+?), Clear all negative effect\(s\) from allies\.$/,
    // --- Fix 6: compound condition patterns ---
    ...
    // --- Fix: by_least_health limit + exclude_from_pool ---
    gainCountDur:                /^Gain (\d+) (.+?) for (\d+) turns\.$/,
    applyCountDurNInjured:       /^Apply (\d+) (.+?) for (\d+) turns to the (\d+) most injured allies\.$/,
    ...
    // --- All-allies targeting patterns (type=random, limit>=10) ---
  ...
  const SENTENCE_TEMPLATES = {
    ...
    fr: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `Obtient ${c} ${pr} pendant ${t} tours.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `Si la cible principale n'est pas ${tr}, draine ${p} % de la vie maximale de la cible.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `Si la cible principale est ${tr}, draine ${p} % de la vie maximale de la cible.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `Sinon, draine ${p} % de la vie maximale de la cible.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'fr')}, si un ennemi a des effets bénéfiques, convertit ${c} effet(s) bénéfique(s) en nuisible(s) sur la cible principale.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `Si ce personnage a plus de ${n} ${proc}, si ce personnage a ${proc2}, réduit la durée de ${proc3} de ${c} sur les alliés.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `Draine ${p} % de la vie maximale de la cible.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct} % de chance d'appliquer ${pr} à la cible principale.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'fr')}, si ${ally} est allié, réduit la jauge de vitesse de ${p} %.` },
        ...
        { match: _P.modeOnCritApply, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'fr')}, sur critique, applique +${c} ${pr} à la cible principale.` },
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'fr')}, sur critique, applique ${pr} à la cible principale.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'fr')}, sur critique, génère +${c} énergie de capacité pour tous les alliés.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'fr')}, sur critique, si ${c}+ alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}, applique ${pr} à la cible principale.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'fr')}, si ${ally} est allié, applique ${pr} à la cible principale.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'fr')}, si ${ally} est allié, +${d} % de dégâts.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'fr')}, si ${ally} est allié, +${d} % de dégâts + ${p} % de dégâts perforants.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'fr')}, si la cible principale a ${proc}, applique +${c} ${pr} à un allié aléatoire.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'fr')}, si ce personnage a ${proc}, +${d} % de dégâts + ${p} % de dégâts perforants.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'fr')}, applique +${c} ${pr} à la cible principale.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'fr')}, si la cible principale a ${proc}, applique ${pr} à la cible principale.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'fr')}, si ${resource} n'est pas plein(e) et si la cible principale est ${trait}, applique ${pr} à la cible principale.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `Sinon, attaque pour ${p}% Perçant.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `Sinon, attaque pour ${d}% dégâts + ${p}% Perçant.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `Sinon, attaque pour ${d}% dégâts.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `Si ce personnage n'a pas ${proc}, obtient +${c} ${pr}, jusqu'à un maximum de ${max}.` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `Si la cible principale a ${proc}, obtient +${c} ${pr}, jusqu'à un maximum de ${max}.` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `En assistance, obtient +${c} ${pr}, jusqu'à un maximum de ${max}.` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `Draine ${p} % de la vie max. de la cible et redistribue aux alliés ${_traitLoc(target, SENTENCE_TEMPLATES._activeDict)}.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `Sur coup critique, obtient +${c} ${pr}, jusqu'à un maximum de ${max}.` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `+${p} % de dégâts perforants à 1 cible adjacente.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `+${d} % de dégâts + ${p} % de dégâts perforants à 1 cible adjacente.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `+${d} % de dégâts à 1 cible adjacente.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `+${p} % de dégâts perforants à jusqu'à ${n} cibles adjacentes.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `+${d} % de dégâts + ${p} % de dégâts perforants à jusqu'à ${n} cibles adjacentes.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `+${d} % de dégâts à jusqu'à ${n} cibles adjacentes.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `+${d} % de dégâts aux ennemis adjacents.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `+${d} % de dégâts + ${p} % de dégâts perforants aux ennemis adjacents.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `+${d} % de dégâts + ${c} % de chances de critique aux ennemis adjacents.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `+${d} % de dégâts + ${dr} % de drain de vie à jusqu'à ${n} cibles adjacentes.` },
        { match: _P.noteChainBrokenCounter, replace: () => `La chaîne est brisée par une contre-attaque.` },
        { match: _P.noteChainBrokenDodge, replace: () => `La chaîne est brisée par une esquive.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'fr')}, +${d} % de dégâts à 1 cible adjacente.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'fr')}, +${d} % de dégâts à jusqu'à ${n} cibles adjacentes.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'fr')}, +${d} % de dégâts à 1 cible adjacente.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `Si ce personnage a plus de ${n} ${proc}, +${d} % de dégâts à 1 cible adjacente.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `Si la cible principale a des effets nuisibles, +${d} % de dégâts à jusqu'à ${n} cibles adjacentes.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `Si la cible principale a des effets nuisibles, copie ${c} effet(s) nuisible(s) de la cible principale, sauf ${excl}.` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d} % de dégâts.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'fr')}, applique +${c} ${pr} à un allié aléatoire.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `Draine ${p} % de la vie max des alliés.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `Si la cible principale a ${proc}, draine ${p} % de la vie max de la cible et redistribue aux ${target}.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `Si ce personnage a moins de ${n} ${proc}, obtient +${c} ${pr}, jusqu'à un maximum de ${max}.` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'fr')}, ${sub}, si la cible principale a ${proc}, draine ${p} % de la vie max de la cible.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'fr')}, ${sub}, si la cible principale n'est pas ${tr}, draine ${p} % de la vie max de la cible.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `Si ce personnage a ${proc}, obtient ${pr}.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `Si ce personnage a ${proc}, perd ${c} ${pr}.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `Si ce personnage a plus de ${n} ${p1}, Si ce personnage a ${p2}, perd ${c} ${pr}.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `Si ${ally} est un allié, applique ${pr} à la cible principale.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `Sinon, si la cible principale a ${proc}, obtient +${c} ${pr}.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `Les ennemis tués par cette attaque ne peuvent pas être ressuscités.` },
        { match: _P.noteAttackCantCountered, replace: () => `Cette attaque ne peut pas être contrée.` },
        { match: _P.noteAttackCantBlocked, replace: () => `Cette attaque ne peut pas être bloquée.` },
        { match: _P.noteAttackCantDodged, replace: () => `Cette attaque ne peut pas être esquivée.` },
        { match: _P.noteAttackCantMiss, replace: () => `Cette attaque ne peut pas rater.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'fr')}, applique ${pr} aux alliés.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `Si ce personnage a ${c} ou moins ${pr}, obtient +${c2} ${pr2}, jusqu'à un maximum de ${mx}.` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `Si la cible principale a ${pr1}, retire ${pr2} de la cible principale.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `Si la cible principale a ${pr}, barrière de ${p} % de la vie max aux alliés.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `Sinon, barrière de ${p} % de la vie max aux alliés.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `Applique ${pr} pendant ${t} tours à un allié ${tr1} ou ${tr2} aléatoire (sauf soi-même).` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `Si l'énergie de capacité n'est pas pleine, applique +${c} ${pr} pendant ${t} tours à la cible principale.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `Si l'énergie de capacité n'est pas pleine, attaque pour ${p} % de dégâts perforants.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'fr')}, si la cible principale a ${pr1}, retire ${pr2} de la cible principale.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'fr')}, retire ${c} effet(s) bénéfique(s) de la cible principale.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'fr')}, les ennemis tués par cette attaque ne peuvent pas être ressuscités.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `Si ce personnage a des effets nuisibles, vole tous les effet(s) nuisible(s) de la cible principale.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `Sinon, vole tous les effet(s) bénéfique(s) de la cible principale, sauf ${pr}.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `En assistance, vole tous les effet(s) bénéfique(s) de la cible principale, sauf ${pr}.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `Si ce personnage n'a pas d'effets nuisibles, ${_notModeLoc(mode, 'fr')}, convertit ${c} effet(s) bénéfique(s) en nuisible(s) sur la cible principale.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `Sinon, ${_notModeLoc(mode, 'fr')}, convertit ${c} effet(s) nuisible(s) en bénéfique(s) sur les alliés.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'fr')}, convertit ${c} effet(s) nuisible(s) en bénéfique(s) sur les alliés.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `Applique ${c} ${pr} pendant ${t} tours aux ${n} alliés les plus blessés.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `Applique ${pr} à tous les alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `Applique ${pr} pendant ${t} tours à tous les alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `Applique +${c} ${pr}, jusqu'à un maximum de ${max} à tous les alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `Soigne tous les alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} de ${p} % de la vie max.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `Barrière de ${p} % de la vie max. à tous les alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `Convertit ${c} effet(s) nuisible(s) en bénéfique(s) sur tous les alliés ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
    ...
    de: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `Erhält ${c} ${pr} für ${t} Runden.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `Wenn das Primärziel kein ${tr} ist, entzieht es ${p} % der max. LP des Ziels.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `Wenn das Primärziel ${tr} ist, entzieht es ${p} % der max. LP des Ziels.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `Andernfalls entzieht es ${p} % der max. LP des Ziels.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'de')}: Wenn ein Feind positive Effekte hat, wandelt ${c} positive(n) Effekt(e) in negative um auf dem Primärziel.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `Wenn man mehr als ${n} ${proc} hat und ${proc2} hat, verringert die Dauer von ${proc3} um ${c} bei Verbündeten.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `Entzieht ${p} % der max. LP des Ziels.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct} % Chance, ${pr} auf das Primärziel anzuwenden.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'de')}: Wenn ${ally} ein Verbündeter ist, verringert die Geschwindigkeitsleiste um ${p} %.` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'de')}: Bei kritischem Treffer wendet ${pr} auf das Primärziel an.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'de')}: Bei kritischem Treffer erzeugt +${c} Fähigkeitsenergie für alle Verbündeten.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'de')}: Bei kritischem Treffer, wenn ${c}+ ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündete, wendet ${pr} auf das Primärziel an.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'de')}: Wenn ${ally} ein Verbündeter ist, wendet ${pr} auf das Primärziel an.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'de')}: Wenn ${ally} ein Verbündeter ist, +${d} % Schaden.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'de')}: Wenn ${ally} ein Verbündeter ist, +${d} % Schaden + ${p} % Durchdringungsschaden.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'de')}: Wenn das Primärziel ${proc} hat, wendet +${c} ${pr} auf einen zufälligen Verbündeten an.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'de')}: Wenn dieser Charakter ${proc} hat, +${d} % Schaden + ${p} % Durchdringungsschaden.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'de')}: Wendet +${c} ${pr} auf das Primärziel an.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'de')}: Wenn das Primärziel ${proc} hat, wendet ${pr} auf das Primärziel an.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'de')}: Wenn ${resource} nicht voll ist und das Primärziel ${trait} ist, wendet ${pr} auf das Primärziel an.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `Andernfalls Angriff für ${p}% Durchdringung.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `Andernfalls Angriff für ${d}% Schaden + ${p}% Durchdringung.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `Andernfalls Angriff für ${d}% Schaden.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `Wenn dieser Charakter kein ${proc} hat, erhält +${c} ${pr}, bis maximal ${max}.` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `Wenn das Primärziel ${proc} hat, erhält +${c} ${pr}, bis maximal ${max}.` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `Bei Assist: Erhält +${c} ${pr}, bis maximal ${max}.` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `Entzieht ${p} % der max. LP des Ziels und verteilt sie an ${_traitLoc(target, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündete.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `Bei kritischem Treffer: Erhält +${c} ${pr}, bis maximal ${max}.` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `+${p} % Durchdringung auf 1 benachbartes Ziel.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `+${d} % Schaden + ${p} % Durchdringung auf 1 benachbartes Ziel.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `+${d} % Schaden auf 1 benachbartes Ziel.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `+${p} % Durchdringung auf bis zu ${n} benachbarte Ziele.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `+${d} % Schaden + ${p} % Durchdringung auf bis zu ${n} benachbarte Ziele.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `+${d} % Schaden auf bis zu ${n} benachbarte Ziele.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `+${d} % Schaden auf benachbarte Feinde.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `+${d} % Schaden + ${p} % Durchdringung auf benachbarte Feinde.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `+${d} % Schaden + ${c} % Krit. Chance auf benachbarte Feinde.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `+${d} % Schaden + ${dr} % Lebensraub auf bis zu ${n} benachbarte Ziele.` },
        { match: _P.noteChainBrokenCounter, replace: () => `Kette wird durch Konter unterbrochen.` },
        { match: _P.noteChainBrokenDodge, replace: () => `Kette wird durch Ausweichen unterbrochen.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'de')}: +${d} % Schaden auf 1 benachbartes Ziel.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'de')}: +${d} % Schaden auf bis zu ${n} benachbarte Ziele.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'de')}: +${d} % Schaden auf 1 benachbartes Ziel.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `Wenn dieser Charakter mehr als ${n} ${proc} hat, +${d} % Schaden auf 1 benachbartes Ziel.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `Wenn das Primärziel negative Effekte hat, +${d} % Schaden auf bis zu ${n} benachbarte Ziele.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `Wenn das Primärziel negative Effekte hat, kopiert ${c} negative(n) Effekt(e) vom Primärziel, außer ${excl}.` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d} % Schaden.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'de')}: Wendet +${c} ${pr} auf einen zufälligen Verbündeten an.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `Entzieht ${p} % der max. Gesundheit von Verbündeten.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `Wenn das Primärziel ${proc} hat, entzieht ${p} % der max. Gesundheit des Ziels und verteilt an ${target}.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `Wenn man weniger als ${n} ${proc} hat, erhält +${c} ${pr}, bis maximal ${max}.` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'de')}, ${sub}: Wenn das Primärziel ${proc} hat, entzieht ${p} % der max. Gesundheit des Ziels.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'de')}, ${sub}: Wenn das Primärziel nicht ${tr} ist, entzieht ${p} % der max. Gesundheit des Ziels.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `Wenn man ${proc} hat: Erhält ${pr}.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `Wenn man ${proc} hat: Verliert ${c} ${pr}.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `Wenn man mehr als ${n} ${p1} hat: Wenn man ${p2} hat: Verliert ${c} ${pr}.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `Wenn ${ally} ein Verbündeter ist: Wendet ${pr} auf das Primärziel an.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `Andernfalls, wenn das Primärziel ${proc} hat: Erhält +${c} ${pr}.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `Durch diesen Angriff getötete Feinde können nicht wiederbelebt werden.` },
        { match: _P.noteAttackCantCountered, replace: () => `Dieser Angriff kann nicht gekontert werden.` },
        { match: _P.noteAttackCantBlocked, replace: () => `Dieser Angriff kann nicht geblockt werden.` },
        { match: _P.noteAttackCantDodged, replace: () => `Diesem Angriff kann nicht ausgewichen werden.` },
        { match: _P.noteAttackCantMiss, replace: () => `Dieser Angriff kann nicht verfehlen.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'de')}: Wende ${pr} auf Verbündete an.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `Wenn selbst ${c} oder weniger ${pr} hat, erhält +${c2} ${pr2}, bis maximal ${mx}.` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `Wenn das Hauptziel ${pr1} hat, entferne ${pr2} vom Hauptziel.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `Wenn das Hauptziel ${pr} hat, Barriere für ${p} % der max. Gesundheit für Verbündete.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `Andernfalls, Barriere für ${p} % der max. Gesundheit für Verbündete.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `Wende ${pr} für ${t} Runden auf einen zufälligen ${tr1} oder ${tr2} Verbündeten an (außer sich selbst).` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `Wenn Fähigkeitsenergie nicht voll ist, wende +${c} ${pr} für ${t} Runden auf das Hauptziel an.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `Wenn Fähigkeitsenergie nicht voll ist, greife stattdessen für ${p} % Durchdringung an.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'de')}: Wenn das Hauptziel ${pr1} hat, entferne ${pr2} vom Hauptziel.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'de')}: Entferne ${c} positive(n) Effekt(e) vom Hauptziel.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'de')}: Von diesem Angriff getötete Gegner können nicht wiederbelebt werden.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `Wenn selbst negative Effekte hat, stehle alle negative(n) Effekt(e) vom Hauptziel.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `Andernfalls, stehle alle positive(n) Effekt(e) vom Hauptziel, außer ${pr}.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `Bei Unterstützung, stehle alle positive(n) Effekt(e) vom Hauptziel, außer ${pr}.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `Wenn selbst keine negativen Effekte hat, ${_notModeLoc(mode, 'de')}: Wandle ${c} positive(n) Effekt(e) in negative auf dem Hauptziel um.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `Andernfalls, ${_notModeLoc(mode, 'de')}: Wandle ${c} negative(n) Effekt(e) in positive auf Verbündeten um.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'de')}: Wandle ${c} negative(n) Effekt(e) in positive auf Verbündeten um.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `Wendet ${c} ${pr} für ${t} Runden auf die ${n} am schwersten verletzten Verbündeten an.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `Wendet ${pr} auf alle ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündeten an.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `Wendet ${pr} für ${t} Runden auf alle ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündeten an.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `Wendet +${c} ${pr} auf alle ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündeten an, bis maximal ${max}.` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `Heilt alle ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündeten um ${p} % der max. LP.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `Barriere für ${p} % der max. Gesundheit für alle ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündeten.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `Wandelt ${c} negative(n) Effekt(e) bei allen ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}-Verbündeten in positive um.` },
    ...
    es: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `Obtiene ${c} ${pr} durante ${t} turnos.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `Si el objetivo principal no es ${tr}, drena ${p} % de la salud máxima del objetivo.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `Si el objetivo principal es ${tr}, drena ${p} % de la salud máxima del objetivo.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `De lo contrario, drena ${p} % de la salud máxima del objetivo.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'es')}, si un enemigo tiene efectos positivos, convierte ${c} efecto(s) positivo(s) en negativo(s) en el objetivo principal.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `Si tiene más de ${n} ${proc} y tiene ${proc2}, reduce la duración de ${proc3} en ${c} en los aliados.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `Drena ${p} % de la salud máxima del objetivo.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct} % de probabilidad de aplicar ${pr} al objetivo principal.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'es')}, si ${ally} es aliado, reduce la barra de velocidad en ${p} %.` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'es')}, con golpe crítico, aplica ${pr} al objetivo principal.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'es')}, con golpe crítico, genera +${c} energía de habilidad para todos los aliados.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'es')}, con golpe crítico, si ${c}+ aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}, aplica ${pr} al objetivo principal.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'es')}, si ${ally} es aliado, aplica ${pr} al objetivo principal.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'es')}, si ${ally} es aliado, +${d} % de daño.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'es')}, si ${ally} es aliado, +${d} % de daño + ${p} % de daño perforante.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'es')}, si el objetivo principal tiene ${proc}, aplica +${c} ${pr} a un aliado aleatorio.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'es')}, si este personaje tiene ${proc}, +${d} % de daño + ${p} % de daño perforante.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'es')}, aplica +${c} ${pr} al objetivo principal.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'es')}, si el objetivo principal tiene ${proc}, aplica ${pr} al objetivo principal.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'es')}, si ${resource} no está lleno y el objetivo principal es ${trait}, aplica ${pr} al objetivo principal.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `De lo contrario, ataca por ${p}% Perforante.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `De lo contrario, ataca por ${d}% daño + ${p}% Perforante.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `De lo contrario, ataca por ${d}% daño.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `Si este personaje no tiene ${proc}, obtiene +${c} ${pr}, hasta un máximo de ${max}.` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `Si el objetivo principal tiene ${proc}, obtiene +${c} ${pr}, hasta un máximo de ${max}.` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `En asistencia, obtiene +${c} ${pr}, hasta un máximo de ${max}.` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `Drena ${p} % de la salud máxima del objetivo y redistribuye a los aliados ${_traitLoc(target, SENTENCE_TEMPLATES._activeDict)}.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `En golpe crítico, obtiene +${c} ${pr}, hasta un máximo de ${max}.` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `+${p} % de penetración a 1 objetivo adyacente.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `+${d} % de daño + ${p} % de penetración a 1 objetivo adyacente.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `+${d} % de daño a 1 objetivo adyacente.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `+${p} % de penetración a hasta ${n} objetivos adyacentes.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `+${d} % de daño + ${p} % de penetración a hasta ${n} objetivos adyacentes.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `+${d} % de daño a hasta ${n} objetivos adyacentes.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `+${d} % de daño a enemigos adyacentes.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `+${d} % de daño + ${p} % de penetración a enemigos adyacentes.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `+${d} % de daño + ${c} % de probabilidad de crítico a enemigos adyacentes.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `+${d} % de daño + ${dr} % de drenaje a hasta ${n} objetivos adyacentes.` },
        { match: _P.noteChainBrokenCounter, replace: () => `La cadena se rompe por contraataque.` },
        { match: _P.noteChainBrokenDodge, replace: () => `La cadena se rompe por evasión.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'es')}, +${d} % de daño a 1 objetivo adyacente.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'es')}, +${d} % de daño a hasta ${n} objetivos adyacentes.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'es')}, +${d} % de daño a 1 objetivo adyacente.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `Si este personaje tiene más de ${n} ${proc}, +${d} % de daño a 1 objetivo adyacente.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `Si el objetivo principal tiene efectos negativos, +${d} % de daño a hasta ${n} objetivos adyacentes.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `Si el objetivo principal tiene efectos negativos, copia ${c} efecto(s) negativo(s) del objetivo principal, excepto ${excl}.` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d} % de daño.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'es')}, aplica +${c} ${pr} a un aliado aleatorio.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `Drena ${p} % de la salud máxima de los aliados.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `Si el objetivo principal tiene ${proc}, drena ${p} % de la salud máxima del objetivo y redistribuye a los ${target}.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `Si tiene menos de ${n} ${proc}, obtiene +${c} ${pr}, hasta un máximo de ${max}.` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'es')}, ${sub}, si el objetivo principal tiene ${proc}, drena ${p} % de la salud máxima del objetivo.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'es')}, ${sub}, si el objetivo principal no es ${tr}, drena ${p} % de la salud máxima del objetivo.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `Si tiene ${proc}, obtiene ${pr}.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `Si tiene ${proc}, pierde ${c} ${pr}.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `Si tiene más de ${n} ${p1}, Si tiene ${p2}, pierde ${c} ${pr}.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `Si ${ally} es un aliado, aplica ${pr} al objetivo principal.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `De lo contrario, si el objetivo principal tiene ${proc}, obtiene +${c} ${pr}.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `Los enemigos eliminados por este ataque no pueden ser resucitados.` },
        { match: _P.noteAttackCantCountered, replace: () => `Este ataque no puede ser contrarrestado.` },
        { match: _P.noteAttackCantBlocked, replace: () => `Este ataque no puede ser bloqueado.` },
        { match: _P.noteAttackCantDodged, replace: () => `Este ataque no puede ser eludido.` },
        { match: _P.noteAttackCantMiss, replace: () => `Este ataque no puede fallar.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'es')}, aplica ${pr} a los aliados.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `Si este personaje tiene ${c} o menos ${pr}, obtiene +${c2} ${pr2}, hasta un máximo de ${mx}.` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `Si el objetivo principal tiene ${pr1}, elimina ${pr2} del objetivo principal.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `Si el objetivo principal tiene ${pr}, barrera del ${p} % de la salud máxima a los aliados.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `De lo contrario, barrera del ${p} % de la salud máxima a los aliados.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `Aplica ${pr} durante ${t} turnos a un aliado ${tr1} o ${tr2} aleatorio (excepto a sí mismo).` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `Si la energía de habilidad no está llena, aplica +${c} ${pr} durante ${t} turnos al objetivo principal.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `Si la energía de habilidad no está llena, ataca con ${p} % de perforación en su lugar.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'es')}, si el objetivo principal tiene ${pr1}, elimina ${pr2} del objetivo principal.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'es')}, elimina ${c} efecto(s) positivo(s) del objetivo principal.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'es')}, los enemigos eliminados por este ataque no pueden ser revividos.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `Si este personaje tiene efectos negativos, roba todos los efecto(s) negativo(s) del objetivo principal.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `De lo contrario, roba todos los efecto(s) positivo(s) del objetivo principal, excepto ${pr}.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `En asistencia, roba todos los efecto(s) positivo(s) del objetivo principal, excepto ${pr}.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `Si este personaje no tiene efectos negativos, ${_notModeLoc(mode, 'es')}, convierte ${c} efecto(s) positivo(s) en negativo(s) en el objetivo principal.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `De lo contrario, ${_notModeLoc(mode, 'es')}, convierte ${c} efecto(s) negativo(s) en positivo(s) en los aliados.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'es')}, convierte ${c} efecto(s) negativo(s) en positivo(s) en los aliados.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `Aplica ${c} ${pr} durante ${t} turnos a los ${n} aliados más heridos.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `Aplica ${pr} a todos los aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `Aplica ${pr} durante ${t} turnos a todos los aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `Aplica +${c} ${pr}, hasta un máximo de ${max} a todos los aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `Cura a todos los aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} en un ${p} % de la salud máxima.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `Barrera del ${p} % de la salud máxima a todos los aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `Convierte ${c} efecto(s) negativo(s) en positivo(s) en todos los aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
    ...
    pt: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `Obtém ${c} ${pr} por ${t} turnos.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `Se o alvo primário não for ${tr}, drena ${p}% da vida máxima do alvo.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `Se o alvo primário for ${tr}, drena ${p}% da vida máxima do alvo.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `Caso contrário, drena ${p}% da vida máxima do alvo.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'pt')}, se um inimigo tiver efeitos positivos, converte ${c} efeito(s) positivo(s) em negativo(s) no alvo primário.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `Se tiver mais de ${n} ${proc} e tiver ${proc2}, reduz a duração de ${proc3} em ${c} nos aliados.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `Drena ${p}% da vida máxima do alvo.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct}% de chance de aplicar ${pr} ao alvo primário.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'pt')}, se ${ally} for aliado, reduz a barra de velocidade em ${p}%.` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'pt')}, em acerto crítico, aplique ${pr} ao alvo primário.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'pt')}, em acerto crítico, gera +${c} energia de habilidade para todos os aliados.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'pt')}, em acerto crítico, se ${c}+ aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}, aplique ${pr} ao alvo primário.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'pt')}, se ${ally} for aliado, aplique ${pr} ao alvo primário.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'pt')}, se ${ally} for aliado, +${d}% de dano.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'pt')}, se ${ally} for aliado, +${d}% de dano + ${p}% de dano perfurante.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'pt')}, se o alvo primário tiver ${proc}, aplique +${c} ${pr} a um aliado aleatório.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'pt')}, se este personagem tiver ${proc}, +${d}% de dano + ${p}% de dano perfurante.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'pt')}, aplique +${c} ${pr} ao alvo primário.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'pt')}, se o alvo primário tiver ${proc}, aplique ${pr} ao alvo primário.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'pt')}, se ${resource} não estiver cheio(a) e o alvo primário for ${trait}, aplique ${pr} ao alvo primário.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `Caso contrário, ataca por ${p}% Perfurante.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `Caso contrário, ataca por ${d}% dano + ${p}% Perfurante.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `Caso contrário, ataca por ${d}% dano.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `Se não tiver ${proc}, obtém +${c} ${pr}, até um máximo de ${max}.` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `Se o alvo primário tiver ${proc}, obtém +${c} ${pr}, até um máximo de ${max}.` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `Em assistência, obtém +${c} ${pr}, até um máximo de ${max}.` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `Drena ${p}% da vida máxima do alvo e redistribui aos aliados ${_traitLoc(target, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `Em crítico, obtém +${c} ${pr}, até um máximo de ${max}.` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `+${p}% de perfuração a 1 alvo adjacente.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `+${d}% de dano + ${p}% de perfuração a 1 alvo adjacente.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `+${d}% de dano a 1 alvo adjacente.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `+${p}% de perfuração a até ${n} alvos adjacentes.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `+${d}% de dano + ${p}% de perfuração a até ${n} alvos adjacentes.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `+${d}% de dano a até ${n} alvos adjacentes.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `+${d}% de dano a inimigos adjacentes.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `+${d}% de dano + ${p}% de perfuração a inimigos adjacentes.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `+${d}% de dano + ${c}% de chance de crítico a inimigos adjacentes.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `+${d}% de dano + ${dr}% de drenagem a até ${n} alvos adjacentes.` },
        { match: _P.noteChainBrokenCounter, replace: () => `A cadeia é interrompida por contra-ataque.` },
        { match: _P.noteChainBrokenDodge, replace: () => `A cadeia é interrompida por esquiva.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'pt')}, +${d}% de dano a 1 alvo adjacente.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'pt')}, +${d}% de dano a até ${n} alvos adjacentes.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'pt')}, +${d}% de dano a 1 alvo adjacente.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `Se tiver mais de ${n} ${proc}, +${d}% de dano a 1 alvo adjacente.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `Se o alvo primário tiver efeitos negativos, +${d}% de dano a até ${n} alvos adjacentes.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `Se o alvo primário tiver efeitos negativos, copia ${c} efeito(s) negativo(s) do alvo primário, exceto ${excl}.` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d}% de dano.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'pt')}, aplica +${c} ${pr} a um aliado aleatório.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `Drena ${p}% da saúde máxima dos aliados.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `Se o alvo primário tiver ${proc}, drena ${p}% da saúde máxima do alvo e redistribui aos ${target}.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `Se tiver menos de ${n} ${proc}, obtém +${c} ${pr}, até um máximo de ${max}.` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'pt')}, ${sub}, se o alvo primário tiver ${proc}, drena ${p}% da saúde máxima do alvo.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'pt')}, ${sub}, se o alvo primário não for ${tr}, drena ${p}% da saúde máxima do alvo.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `Se tiver ${proc}, obtém ${pr}.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `Se tiver ${proc}, perde ${c} ${pr}.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `Se tiver mais de ${n} ${p1}, Se tiver ${p2}, perde ${c} ${pr}.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `Se ${ally} for aliado, aplica ${pr} ao alvo primário.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `Caso contrário, se o alvo primário tiver ${proc}, obtém +${c} ${pr}.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `Inimigos eliminados por este ataque não podem ser ressuscitados.` },
        { match: _P.noteAttackCantCountered, replace: () => `Este ataque não pode ser contra-atacado.` },
        { match: _P.noteAttackCantBlocked, replace: () => `Este ataque não pode ser bloqueado.` },
        { match: _P.noteAttackCantDodged, replace: () => `Este ataque não pode ser esquivado.` },
        { match: _P.noteAttackCantMiss, replace: () => `Este ataque não pode errar.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'pt')}, aplica ${pr} aos aliados.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `Se este personagem tiver ${c} ou menos ${pr}, obtém +${c2} ${pr2}, até um máximo de ${mx}.` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `Se o alvo principal tiver ${pr1}, remove ${pr2} do alvo principal.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `Se o alvo principal tiver ${pr}, barreira de ${p}% da saúde máxima aos aliados.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `Caso contrário, barreira de ${p}% da saúde máxima aos aliados.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `Aplica ${pr} por ${t} turnos a um aliado ${tr1} ou ${tr2} aleatório (exceto a si mesmo).` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `Se a energia de habilidade não estiver cheia, aplica +${c} ${pr} por ${t} turnos ao alvo principal.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `Se a energia de habilidade não estiver cheia, ataca com ${p}% de perfuração em vez disso.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'pt')}, se o alvo principal tiver ${pr1}, remove ${pr2} do alvo principal.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'pt')}, remove ${c} efeito(s) positivo(s) do alvo principal.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'pt')}, inimigos eliminados por este ataque não podem ser revividos.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `Se este personagem tiver efeitos negativos, rouba todos os efeito(s) negativo(s) do alvo principal.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `Caso contrário, rouba todos os efeito(s) positivo(s) do alvo principal, exceto ${pr}.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `Em assistência, rouba todos os efeito(s) positivo(s) do alvo principal, exceto ${pr}.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `Se este personagem não tiver efeitos negativos, ${_notModeLoc(mode, 'pt')}, converte ${c} efeito(s) positivo(s) em negativo(s) no alvo principal.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `Caso contrário, ${_notModeLoc(mode, 'pt')}, converte ${c} efeito(s) negativo(s) em positivo(s) nos aliados.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'pt')}, converte ${c} efeito(s) negativo(s) em positivo(s) nos aliados.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `Aplica ${c} ${pr} por ${t} turnos aos ${n} aliados mais feridos.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `Aplica ${pr} a todos os aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `Aplica ${pr} por ${t} turnos a todos os aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `Aplique +${c} ${pr}, até um máximo de ${max} a todos os aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `Cura todos os aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()} em ${p}% da vida máxima.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `Barreira de ${p}% da saúde máxima a todos os aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `Converte ${c} efeito(s) negativo(s) em positivo(s) em todos os aliados ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
    ...
    it: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `Ottiene ${c} ${pr} per ${t} turni.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `Se il bersaglio primario non è ${tr}, prosciuga ${p}% della Salute massima del bersaglio.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `Se il bersaglio primario è ${tr}, prosciuga ${p}% della Salute massima del bersaglio.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `Altrimenti, prosciuga ${p}% della Salute massima del bersaglio.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'it')}, se un nemico ha effetti positivi, converte ${c} effetto/i positivo/i in negativo/i sul bersaglio primario.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `Se ha più di ${n} ${proc} e ha ${proc2}, riduce la durata di ${proc3} di ${c} sugli alleati.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `Drena ${p}% della salute massima del bersaglio.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct}% di probabilità di applicare ${pr} al bersaglio primario.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'it')}, se ${ally} è un alleato, riduce la barra velocità del ${p}%.` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'it')}, su colpo critico, applica ${pr} al bersaglio primario.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'it')}, su colpo critico, genera +${c} energia abilità per tutti gli alleati.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'it')}, su colpo critico, se ${c}+ alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}, applica ${pr} al bersaglio primario.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'it')}, se ${ally} è un alleato, applica ${pr} al bersaglio primario.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'it')}, se ${ally} è un alleato, +${d}% danno.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'it')}, se ${ally} è un alleato, +${d}% danno + ${p}% danno perforante.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'it')}, se il bersaglio primario ha ${proc}, applica +${c} ${pr} a un alleato casuale.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'it')}, se questo personaggio ha ${proc}, +${d}% danno + ${p}% danno perforante.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'it')}, applica +${c} ${pr} al bersaglio primario.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'it')}, se il bersaglio primario ha ${proc}, applica ${pr} al bersaglio primario.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'it')}, se ${resource} non è pieno/a e il bersaglio primario è ${trait}, applica ${pr} al bersaglio primario.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `Altrimenti, attacca per ${p}% Perforante.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `Altrimenti, attacca per ${d}% danni + ${p}% Perforante.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `Altrimenti, attacca per ${d}% danni.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `Se questo personaggio non ha ${proc}, ottiene +${c} ${pr}, fino a un massimo di ${max}.` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `Se il bersaglio primario ha ${proc}, ottiene +${c} ${pr}, fino a un massimo di ${max}.` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `In assistenza, ottiene +${c} ${pr}, fino a un massimo di ${max}.` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `Drena il ${p}% della salute massima del bersaglio e ridistribuisce agli alleati ${_traitLoc(target, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `In colpo critico, ottiene +${c} ${pr}, fino a un massimo di ${max}.` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `+${p}% di Perforazione a 1 bersaglio adiacente.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `+${d}% di danno + ${p}% di Perforazione a 1 bersaglio adiacente.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `+${d}% di danno a 1 bersaglio adiacente.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `+${p}% di Perforazione a fino a ${n} bersagli adiacenti.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `+${d}% di danno + ${p}% di Perforazione a fino a ${n} bersagli adiacenti.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `+${d}% di danno a fino a ${n} bersagli adiacenti.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `+${d}% di danno ai nemici adiacenti.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `+${d}% di danno + ${p}% di Perforazione ai nemici adiacenti.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `+${d}% di danno + ${c}% di probabilità di critico ai nemici adiacenti.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `+${d}% di danno + ${dr}% di Drenaggio a fino a ${n} bersagli adiacenti.` },
        { match: _P.noteChainBrokenCounter, replace: () => `La catena viene interrotta dal contrattacco.` },
        { match: _P.noteChainBrokenDodge, replace: () => `La catena viene interrotta dalla schivata.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'it')}, +${d}% di danno a 1 bersaglio adiacente.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'it')}, +${d}% di danno a fino a ${n} bersagli adiacenti.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'it')}, +${d}% di danno a 1 bersaglio adiacente.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `Se questo personaggio ha più di ${n} ${proc}, +${d}% di danno a 1 bersaglio adiacente.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `Se il bersaglio primario ha effetti negativi, +${d}% di danno a fino a ${n} bersagli adiacenti.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `Se il bersaglio primario ha effetti negativi, copia ${c} effetto/i negativo/i dal bersaglio primario, escluso ${excl}.` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d}% di danno.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'it')}, applica +${c} ${pr} a un alleato casuale.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `Drena il ${p}% della salute massima degli alleati.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `Se il bersaglio primario ha ${proc}, drena il ${p}% della salute massima del bersaglio e redistribuisce ai ${target}.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `Se ha meno di ${n} ${proc}, ottiene +${c} ${pr}, fino a un massimo di ${max}.` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'it')}, ${sub}, se il bersaglio primario ha ${proc}, drena il ${p}% della salute massima del bersaglio.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'it')}, ${sub}, se il bersaglio primario non è ${tr}, drena il ${p}% della salute massima del bersaglio.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `Se ha ${proc}, ottiene ${pr}.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `Se ha ${proc}, perde ${c} ${pr}.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `Se ha più di ${n} ${p1}, Se ha ${p2}, perde ${c} ${pr}.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `Se ${ally} è un alleato, applica ${pr} al bersaglio primario.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `Altrimenti, se il bersaglio primario ha ${proc}, ottiene +${c} ${pr}.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `I nemici uccisi da questo attacco non possono essere rianimati.` },
        { match: _P.noteAttackCantCountered, replace: () => `Questo attacco non può essere contrattaccato.` },
        { match: _P.noteAttackCantBlocked, replace: () => `Questo attacco non può essere bloccato.` },
        { match: _P.noteAttackCantDodged, replace: () => `Questo attacco non può essere schivato.` },
        { match: _P.noteAttackCantMiss, replace: () => `Questo attacco non può mancare.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'it')}, applica ${pr} agli alleati.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `Se questo personaggio ha ${c} o meno ${pr}, ottiene +${c2} ${pr2}, fino a un massimo di ${mx}.` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `Se il bersaglio principale ha ${pr1}, rimuovi ${pr2} dal bersaglio principale.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `Se il bersaglio principale ha ${pr}, barriera del ${p}% della salute massima agli alleati.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `Altrimenti, barriera del ${p}% della salute massima agli alleati.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `Applica ${pr} per ${t} turni a un alleato ${tr1} o ${tr2} casuale (escluso sé stesso).` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `Se l'energia abilità non è piena, applica +${c} ${pr} per ${t} turni al bersaglio principale.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `Se l'energia abilità non è piena, attacca con ${p}% di perforazione.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'it')}, se il bersaglio principale ha ${pr1}, rimuovi ${pr2} dal bersaglio principale.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'it')}, rimuovi ${c} effetto/i positivo/i dal bersaglio principale.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'it')}, i nemici uccisi da questo attacco non possono essere rianimati.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `Se questo personaggio ha effetti negativi, ruba tutti gli effetto/i negativo/i dal bersaglio principale.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `Altrimenti, ruba tutti gli effetto/i positivo/i dal bersaglio principale, esclusi ${pr}.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `In assistenza, ruba tutti gli effetto/i positivo/i dal bersaglio principale, esclusi ${pr}.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `Se questo personaggio non ha effetti negativi, ${_notModeLoc(mode, 'it')}, converte ${c} effetto/i positivo/i in negativo/i sul bersaglio principale.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `Altrimenti, ${_notModeLoc(mode, 'it')}, converte ${c} effetto/i negativo/i in positivo/i sugli alleati.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'it')}, converte ${c} effetto/i negativo/i in positivo/i sugli alleati.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `Applica ${c} ${pr} per ${t} turni ai ${n} alleati più feriti.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `Applica ${pr} a tutti gli alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `Applica ${pr} per ${t} turni a tutti gli alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `Applica +${c} ${pr}, fino a un massimo di ${max} a tutti gli alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `Cura tutti gli alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()} del ${p}% della salute massima.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `Barriera del ${p}% della salute massima a tutti gli alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `Converte ${c} effetto/i negativo/i in positivo/i su tutti gli alleati ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict).toUpperCase()}.` },
    ...
    ja: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `${c}${pr}を${t}ターン獲得。` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `メインターゲットが${tr}でない場合、ターゲットの最大体力の${p}%を吸収。` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `メインターゲットが${tr}の場合、ターゲットの最大体力の${p}%を吸収。` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `それ以外の場合、ターゲットの最大体力の${p}%を吸収。` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'ja')}、敵がポジティブ効果を持っていれば、メインターゲットのポジティブ効果${c}個をネガティブに変換。` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `${proc}が${n}個以上あり、${proc2}を持っている場合、味方の${proc3}の持続時間を${c}減少。` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `ターゲットの最大体力の${p}%を吸収。` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct}%の確率でメインターゲットに${pr}を適用。` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'ja')}、${ally}が味方なら、スピードバーを${p}%減少。` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'ja')}、クリティカル時、メインターゲットに${pr}を適用。` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'ja')}、クリティカル時、全味方に+${c}アビリティエネルギーを生成。` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'ja')}、クリティカル時、${c}+の${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方がいれば、メインターゲットに${pr}を適用。` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'ja')}、${ally}が味方なら、メインターゲットに${pr}を適用。` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'ja')}、${ally}が味方なら、+${d}%ダメージ。` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'ja')}、${ally}が味方なら、+${d}%ダメージ + ${p}%貫通ダメージ。` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'ja')}、メインターゲットが${proc}を持っていれば、ランダムな味方に+${c}${pr}を適用。` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'ja')}、このキャラクターが${proc}を持っていれば、+${d}%ダメージ + ${p}%貫通ダメージ。` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'ja')}、メインターゲットに+${c}${pr}を適用。` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'ja')}、メインターゲットが${proc}を持っていれば、${pr}をメインターゲットに適用。` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'ja')}、${resource}が満タンでなく、メインターゲットが${trait}の場合、${pr}をメインターゲットに適用。` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `それ以外は、${p}%の貫通で攻撃。` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `それ以外は、${d}%ダメージ + ${p}%貫通で攻撃。` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `それ以外は、${d}%ダメージで攻撃。` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `このキャラクターが${proc}を持っていなければ、+${c}${pr}を獲得（最大${max}）。` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `メインターゲットが${proc}を持っている場合、+${c}${pr}を獲得（最大${max}）。` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `アシスト時、+${c}${pr}を獲得（最大${max}）。` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `ターゲットの最大体力の${p}%を吸収し、${_traitLoc(target, SENTENCE_TEMPLATES._activeDict)}の味方に再分配。` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `クリティカル時、+${c}${pr}を獲得（最大${max}）。` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `隣接する1体に+${p}%の貫通効果。` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `隣接する1体に+${d}%のダメージ + ${p}%の貫通効果。` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `隣接する1体に+${d}%のダメージ。` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `隣接する最大${n}体に+${p}%の貫通効果。` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `隣接する最大${n}体に+${d}%のダメージ + ${p}%の貫通効果。` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `隣接する最大${n}体に+${d}%のダメージ。` },
        { match: _P.dmgAdjacent, replace: (m, d) => `隣接する敵に+${d}%のダメージ。` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `隣接する敵に+${d}%のダメージ + ${p}%の貫通効果。` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `隣接する敵に+${d}%のダメージ + ${c}%のクリティカル率。` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `隣接する最大${n}体に+${d}%のダメージ + ${dr}%の吸収効果。` },
        { match: _P.noteChainBrokenCounter, replace: () => `チェーンはカウンターで中断される。` },
        { match: _P.noteChainBrokenDodge, replace: () => `チェーンは回避で中断される。` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'ja')}、隣接する1体に+${d}%のダメージ。` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'ja')}、隣接する最大${n}体に+${d}%のダメージ。` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'ja')}、隣接する1体に+${d}%のダメージ。` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `このキャラクターが${n}を超える${proc}を持っている場合、隣接する1体に+${d}%のダメージ。` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `メインターゲットがネガティブ効果を持っている場合、隣接する最大${n}体に+${d}%のダメージ。` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `メインターゲットがネガティブ効果を持っている場合、メインターゲットのネガティブ効果${c}個をコピー（${excl}を除く）。` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d}%のダメージ。` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'ja')}、ランダムな味方に+${c} ${pr}を適用。` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `味方の最大体力の${p}%を吸収。` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `メインターゲットが${proc}を持っている場合、ターゲットの最大体力の${p}%を吸収し${target}に再分配。` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `${proc}が${n}未満の場合、+${c} ${pr}を獲得（最大${max}）。` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'ja')}、${sub}、メインターゲットが${proc}を持っている場合、ターゲットの最大体力の${p}%を吸収。` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'ja')}、${sub}、メインターゲットが${tr}でない場合、ターゲットの最大体力の${p}%を吸収。` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `${proc}を持っている場合、${pr}を獲得。` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `${proc}を持っている場合、${pr}を${c}失う。` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `${p1}が${n}を超える場合、${p2}を持っている場合、${pr}を${c}失う。` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `${ally}が味方の場合、メインターゲットに${pr}を適用。` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `それ以外で、メインターゲットが${proc}を持っている場合、+${c} ${pr}を獲得。` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `この攻撃で倒された敵は復活不可。` },
        { match: _P.noteAttackCantCountered, replace: () => `この攻撃はカウンター不可。` },
        { match: _P.noteAttackCantBlocked, replace: () => `この攻撃はブロック不可。` },
        { match: _P.noteAttackCantDodged, replace: () => `この攻撃は身かわし不可。` },
        { match: _P.noteAttackCantMiss, replace: () => `この攻撃は必中。` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'ja')}、味方に${pr}を付与。` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `自分の${pr}が${c}以下の場合、+${c2} ${pr2}を獲得（最大${mx}）。` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `メインターゲットが${pr1}を持つ場合、メインターゲットから${pr2}を除去。` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `メインターゲットが${pr}を持つ場合、味方に最大体力の${p}%のバリアを付与。` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `それ以外の場合、味方に最大体力の${p}%のバリアを付与。` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `ランダムな${tr1}または${tr2}味方（自分以外）に${pr}を${t}ターン付与。` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `アビリティエネルギーが満タンでない場合、メインターゲットに+${c} ${pr}を${t}ターン付与。` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `アビリティエネルギーが満タンでない場合、代わりに${p}%パーシングで攻撃。` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'ja')}、メインターゲットが${pr1}を持つ場合、メインターゲットから${pr2}を除去。` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'ja')}、メインターゲットから${c}個のバフを除去。` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'ja')}、この攻撃で倒された敵は復活できない。` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `自分がデバフを持つ場合、メインターゲットから全てのデバフを奪取。` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `それ以外の場合、メインターゲットから全てのバフを奪取、${pr}を除く。` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `アシスト時、メインターゲットから全てのバフを奪取、${pr}を除く。` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `自分がデバフを持たない場合、${_notModeLoc(mode, 'ja')}、メインターゲットの${c}個のバフをデバフに変換。` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `それ以外の場合、${_notModeLoc(mode, 'ja')}、味方の${c}個のデバフをバフに変換。` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'ja')}、味方の${c}個のデバフをバフに変換。` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `最も負傷した${n}人の味方に${c}${pr}を${t}ターン適用。` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `すべての${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方に${pr}を適用。` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `すべての${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方に${pr}を${t}ターン適用。` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `すべての${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方に+${c}${pr}を適用（最大${max}）。` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `すべての${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方を最大体力の${p}%回復。` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `すべての${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方に最大体力の${p}%のバリアを付与。` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `すべての${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}の味方のネガティブ効果を${c}個ポジティブに変換。` },
    ...
    ko: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `${c} ${pr}을 ${t}턴간 획득.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `주 공격 대상이 ${tr}(이)가 아니면 대상 최대 체력의 ${p}%를 흡수합니다.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `주 공격 대상이 ${tr}이면 대상 최대 체력의 ${p}%를 흡수합니다.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `그렇지 않으면 대상 최대 체력의 ${p}%를 흡수합니다.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'ko')}, 적이 긍정적 효과를 가지고 있으면, 주요 대상의 긍정적 효과 ${c}개를 부정적 효과로 전환합니다.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `${proc}을 ${n}개 이상 보유하고 ${proc2}을 가지고 있으면, 아군의 ${proc3} 지속 시간을 ${c} 감소시킵니다.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `대상 최대 체력의 ${p}%를 흡수합니다.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct}% 확률로 주요 대상에게 ${pr}을 적용합니다.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'ko')}, ${ally}이 아군이면, 속도 바를 ${p}% 감소시킵니다.` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'ko')}, 치명타 시, 주요 대상에게 ${pr}을 적용합니다.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'ko')}, 치명타 시, 모든 아군에게 +${c} 능력 에너지를 생성합니다.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'ko')}, 치명타 시, ${c}+명의 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군이 있으면, 주요 대상에게 ${pr}을 적용합니다.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'ko')}, ${ally}이 아군이면, 주요 대상에게 ${pr}을 적용합니다.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'ko')}, ${ally}이 아군이면, +${d}% 피해.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'ko')}, ${ally}이 아군이면, +${d}% 피해 + ${p}% 관통 피해.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'ko')}, 주요 대상이 ${proc}을 가지고 있으면, 무작위 아군에게 +${c} ${pr}을 적용합니다.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'ko')}, 이 캐릭터가 ${proc}을 가지고 있으면, +${d}% 피해 + ${p}% 관통 피해.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'ko')}, 주요 대상에게 +${c} ${pr}을 적용합니다.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'ko')}, 주요 대상이 ${proc}을 가지고 있으면, ${pr}을 주요 대상에게 적용합니다.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'ko')}, ${resource}이(가) 가득 차지 않았고 주요 대상이 ${trait}이면, ${pr}을 주요 대상에게 적용합니다.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `그렇지 않으면, ${p}% 관통으로 공격합니다.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `그렇지 않으면, ${d}% 피해 + ${p}% 관통으로 공격합니다.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `그렇지 않으면, ${d}% 피해로 공격합니다.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `이 캐릭터가 ${proc}을 가지고 있지 않으면 +${c} ${pr} 획득 (최대 ${max}).` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `주요 대상이 ${proc}을 보유하면 +${c} ${pr} 획득 (최대 ${max}).` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `지원 시, +${c} ${pr} 획득 (최대 ${max}).` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `대상의 최대 체력의 ${p}%를 흡수하여 ${_traitLoc(target, SENTENCE_TEMPLATES._activeDict)} 아군에게 재분배합니다.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `치명타 시, +${c} ${pr} 획득 (최대 ${max}).` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `인접한 1명에게 +${p}% 관통 대미지.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `인접한 1명에게 +${d}% 대미지 + ${p}% 관통 대미지.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `인접한 1명에게 +${d}% 대미지.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `인접한 최대 ${n}명에게 +${p}% 관통 대미지.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `인접한 최대 ${n}명에게 +${d}% 대미지 + ${p}% 관통 대미지.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `인접한 최대 ${n}명에게 +${d}% 대미지.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `인접한 적에게 +${d}% 대미지.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `인접한 적에게 +${d}% 대미지 + ${p}% 관통 대미지.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `인접한 적에게 +${d}% 대미지 + ${c}% 치명타 확률.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `인접한 최대 ${n}명에게 +${d}% 대미지 + ${dr}% 흡수.` },
        { match: _P.noteChainBrokenCounter, replace: () => `체인이 반격으로 중단됩니다.` },
        { match: _P.noteChainBrokenDodge, replace: () => `체인이 회피로 중단됩니다.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'ko')}, 인접한 1명에게 +${d}% 대미지.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'ko')}, 인접한 최대 ${n}명에게 +${d}% 대미지.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'ko')}, 인접한 1명에게 +${d}% 대미지.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `이 캐릭터가 ${n}을 초과하는 ${proc}을 가지고 있으면 인접한 1명에게 +${d}% 대미지.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `주요 대상이 부정적 효과를 보유하면, 인접한 최대 ${n}명에게 +${d}% 대미지.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `주요 대상이 부정적 효과를 보유하면, 주요 대상의 부정적 효과 ${c}개를 복사합니다 (${excl} 제외).` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d}% 대미지.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'ko')}, 무작위 아군에게 +${c} ${pr}을 적용합니다.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `아군 최대 체력의 ${p}%를 흡수합니다.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `주요 대상이 ${proc}을 보유하면, 대상 최대 체력의 ${p}%를 흡수하여 ${target}에게 재분배합니다.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `${proc}이 ${n}개 미만이면, +${c} ${pr} 획득 (최대 ${max}).` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'ko')}, ${sub}, 주요 대상이 ${proc}을 보유하면, 대상 최대 체력의 ${p}%를 흡수합니다.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'ko')}, ${sub}, 주요 대상이 ${tr}이 아니면, 대상 최대 체력의 ${p}%를 흡수합니다.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `${proc}을 보유하면, ${pr}을 획득합니다.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `${proc}을 보유하면, ${pr}을 ${c} 잃습니다.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `${p1}이 ${n} 이상이면, ${p2}을 보유하면, ${pr}을 ${c} 잃습니다.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `${ally}이 아군이면, 주요 대상에게 ${pr}을 적용합니다.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `그렇지 않으면, 주요 대상이 ${proc}을 보유하면, +${c} ${pr} 획득.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `이 공격으로 처치된 적은 부활할 수 없습니다.` },
        { match: _P.noteAttackCantCountered, replace: () => `이 공격은 반격할 수 없습니다.` },
        { match: _P.noteAttackCantBlocked, replace: () => `이 공격은 차단할 수 없습니다.` },
        { match: _P.noteAttackCantDodged, replace: () => `이 공격은 회피할 수 없습니다.` },
        { match: _P.noteAttackCantMiss, replace: () => `이 공격은 빗나갈 수 없습니다.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'ko')}, 아군에게 ${pr} 적용.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `자신의 ${pr}이(가) ${c} 이하인 경우, +${c2} ${pr2} 획득 (최대 ${mx}).` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `주요 대상이 ${pr1}을(를) 보유한 경우, 주요 대상에서 ${pr2} 제거.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `주요 대상이 ${pr}을(를) 보유한 경우, 아군에게 최대 체력의 ${p}% 보호막 부여.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `그렇지 않으면, 아군에게 최대 체력의 ${p}% 보호막 부여.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `무작위 ${tr1} 또는 ${tr2} 아군에게 (자신 제외) ${t}턴 동안 ${pr} 적용.` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `능력 에너지가 가득 차지 않은 경우, 주요 대상에게 ${t}턴 동안 +${c} ${pr} 적용.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `능력 에너지가 가득 차지 않은 경우, 대신 ${p}% 관통으로 공격.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'ko')}, 주요 대상이 ${pr1}을(를) 보유한 경우, 주요 대상에서 ${pr2} 제거.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'ko')}, 주요 대상에서 ${c}개의 긍정적 효과 제거.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'ko')}, 이 공격으로 처치된 적은 부활할 수 없습니다.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `자신이 부정적 효과를 보유한 경우, 주요 대상의 모든 부정적 효과를 훔칩니다.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `그렇지 않으면, 주요 대상의 모든 긍정적 효과를 훔칩니다, ${pr} 제외.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `지원 시, 주요 대상의 모든 긍정적 효과를 훔칩니다, ${pr} 제외.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `자신이 부정적 효과가 없는 경우, ${_notModeLoc(mode, 'ko')}, 주요 대상의 ${c}개의 긍정적 효과를 부정적으로 전환.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `그렇지 않으면, ${_notModeLoc(mode, 'ko')}, 아군의 ${c}개의 부정적 효과를 긍정적으로 전환.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'ko')}, 아군의 ${c}개의 부정적 효과를 긍정적으로 전환.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `가장 부상당한 ${n}명의 아군에게 ${c} ${pr}을 ${t}턴간 적용.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `모든 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군에게 ${pr}을 적용합니다.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `모든 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군에게 ${pr}을 ${t}턴 동안 적용합니다.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `모든 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군에게 +${c} ${pr}을 적용합니다 (최대 ${max}).` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `모든 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군을 최대 체력의 ${p}% 회복합니다.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `모든 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군에게 최대 체력의 ${p}% 보호막을 부여합니다.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `모든 ${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} 아군의 부정적 효과 ${c}개를 긍정적으로 변환합니다.` },
    ...
    ru: {
      ...
      patterns: [
        ...
        { match: _P.gainCountDur, replace: (m, c, pr, t) => `Получает ${c} ${pr} на ${t} ходов.` },
        ...
        { match: _P.targetNotTraitDeal, replace: (m, tr, p) => `Если основная цель не является ${tr}, высасывает ${p} % от макс. здоровья цели.` },
        { match: _P.targetTraitDeal, replace: (m, tr, p) => `Если основная цель является ${tr}, высасывает ${p} % от макс. здоровья цели.` },
        ...
        { match: _P.otherwiseDeal, replace: (m, p) => `В противном случае высасывает ${p} % от макс. здоровья цели.` },
        ...
        { match: _P.modeAnyEnemyHasFlip, replace: (m, mode, c) => `${_modeLoc(mode, 'ru')}: если у противника есть положительные эффекты, преобразует ${c} положительный/ых эффект/ов в отрицательные на основной цели.` },
        ...
        { match: _P.selfHasMoreThanSelfHasReduceDurAllies, replace: (m, n, proc, proc2, proc3, c) => `Если у персонажа более ${n} ${proc} и есть ${proc2}, уменьшает длительность ${proc3} на ${c} у союзников.` },
        ...
        { match: _P.dealFlat, replace: (m, p) => `Поглощает ${p}% от макс. здоровья цели.` },
        { match: _P.chanceApply, replace: (m, pct, pr) => `${pct}% шанс применить ${pr} к основной цели.` },
        { match: _P.modeIfAllyReduceSpeed, replace: (m, mode, ally, p) => `${_modeLoc(mode, 'ru')}: если ${ally} союзник, уменьшает шкалу скорости на ${p}%.` },
        ...
        { match: _P.modeOnCritApplyProc, replace: (m, mode, pr) => `${_modeLoc(mode, 'ru')}: при критическом ударе применяет ${pr} к основной цели.` },
        { match: _P.modeOnCritGenEnergyAllAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'ru')}: при критическом ударе генерирует +${c} энергии способностей для всех союзников.` },
        { match: _P.modeOnCritIfTraitAlliesApplyProc, replace: (m, mode, c, tr, pr) => `${_modeLoc(mode, 'ru')}: при критическом ударе, если ${c}+ союзников-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}, применяет ${pr} к основной цели.` },
        { match: _P.modeIfAllyApplyProc, replace: (m, mode, ally, pr) => `${_modeLoc(mode, 'ru')}: если ${ally} союзник, применяет ${pr} к основной цели.` },
        { match: _P.modeIfAllyDmg, replace: (m, mode, ally, d) => `${_modeLoc(mode, 'ru')}: если ${ally} союзник, +${d}% урона.` },
        { match: _P.modeIfAllyDmgPierce, replace: (m, mode, ally, d, p) => `${_modeLoc(mode, 'ru')}: если ${ally} союзник, +${d}% урона + ${p}% пробивающего урона.` },
        { match: _P.modeTargetHasApplyPlusRandomAlly, replace: (m, mode, proc, c, pr) => `${_modeLoc(mode, 'ru')}: если у основной цели есть ${proc}, применяет +${c} ${pr} к случайному союзнику.` },
        { match: _P.modeSelfHasDmgPierce, replace: (m, mode, proc, d, p) => `${_modeLoc(mode, 'ru')}: если у этого персонажа есть ${proc}, +${d}% урона + ${p}% пробивающего урона.` },
        { match: _P.modeApplyPlus, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'ru')}: применяет +${c} ${pr} к основной цели.` },
        { match: _P.modeTargetHasApply, replace: (m, mode, proc, pr) => `${_modeLoc(mode, 'ru')}: если у основной цели есть ${proc}, применяет ${pr} к основной цели.` },
        { match: _P.modeIfNotFullIfTargetIsApply, replace: (m, mode, resource, trait, pr) => `${_modeLoc(mode, 'ru')}: если ${resource} не полна и основная цель — ${trait}, применяет ${pr} к основной цели.` },
        ...
        { match: _P.otherwiseAttackPierce, replace: (m, p) => `В противном случае атакует на ${p}% Пробивания.` },
        { match: _P.otherwiseAttackDmgPierce, replace: (m, d, p) => `В противном случае атакует на ${d}% урона + ${p}% Пробивания.` },
        { match: _P.otherwiseAttackDmg, replace: (m, d) => `В противном случае атакует на ${d}% урона.` },
        ...
        { match: _P.selfNotHasGainMax, replace: (m, proc, c, pr, max) => `Если у этого персонажа нет ${proc}, получает +${c} ${pr}, до максимума ${max}.` },
        ...
        { match: _P.targetHasGainPlusMax, replace: (m, proc, c, pr, max) => `Если у основной цели есть ${proc}, получает +${c} ${pr}, до максимума ${max}.` },
        ...
        { match: _P.onAssistGainPlusMax, replace: (m, c, pr, max) => `При поддержке: получает +${c} ${pr}, до максимума ${max}.` },
        ...
        { match: _P.dealRedistribute, replace: (m, p, target) => `Высасывает ${p} % от макс. здоровья цели и перераспределяет союзникам-${_traitLoc(target, SENTENCE_TEMPLATES._activeDict)}.` },
        ...
        { match: _P.onCritGainPlusMax, replace: (m, c, pr, max) => `При крит. ударе: получает +${c} ${pr}, до максимума ${max}.` },
        ...
        // Chain targeting
        { match: _P.pierceAdjacentOne, replace: (m, p) => `+${p} % проникающего урона по 1 соседней цели.` },
        { match: _P.dmgPierceAdjacentOne, replace: (m, d, p) => `+${d} % урона + ${p} % проникающего урона по 1 соседней цели.` },
        { match: _P.dmgAdjacentOne, replace: (m, d) => `+${d} % урона по 1 соседней цели.` },
        { match: _P.pierceAdjacentN, replace: (m, p, n) => `+${p} % проникающего урона по до ${n} соседних целей.` },
        { match: _P.dmgPierceAdjacentN, replace: (m, d, p, n) => `+${d} % урона + ${p} % проникающего урона по до ${n} соседних целей.` },
        { match: _P.dmgAdjacentN, replace: (m, d, n) => `+${d} % урона по до ${n} соседних целей.` },
        { match: _P.dmgAdjacent, replace: (m, d) => `+${d} % урона по соседним врагам.` },
        { match: _P.dmgPierceAdjacent, replace: (m, d, p) => `+${d} % урона + ${p} % проникающего урона по соседним врагам.` },
        { match: _P.dmgCritChanceAdjacent, replace: (m, d, c) => `+${d} % урона + ${c} % шанса крит. удара по соседним врагам.` },
        { match: _P.dmgDrainAdjacentN, replace: (m, d, dr, n) => `+${d} % урона + ${dr} % высасывания по до ${n} соседних целей.` },
        { match: _P.noteChainBrokenCounter, replace: () => `Цепочка прерывается контратакой.` },
        { match: _P.noteChainBrokenDodge, replace: () => `Цепочка прерывается уклонением.` },
        { match: _P.modeDmgAdjacentOne, replace: (m, mode, d) => `${_modeLoc(mode, 'ru')}: +${d} % урона по 1 соседней цели.` },
        { match: _P.modeDmgAdjacentN, replace: (m, mode, d, n) => `${_modeLoc(mode, 'ru')}: +${d} % урона по до ${n} соседних целей.` },
        { match: _P.notModeDmgAdjacentOne, replace: (m, mode, d) => `${_notModeLoc(mode, 'ru')}: +${d} % урона по 1 соседней цели.` },
        { match: _P.selfHasMoreDmgAdjacentOne, replace: (m, n, proc, d) => `Если у этого персонажа более ${n} ${proc}, +${d} % урона по 1 соседней цели.` },
        { match: _P.targetHasAnyNegDmgAdjacentN, replace: (m, d, n) => `Если у основной цели есть отрицательные эффекты, +${d} % урона по до ${n} соседних целей.` },
        { match: _P.targetHasAnyNegCopyNegExcl, replace: (m, c, excl) => `Если у основной цели есть отрицательные эффекты, копирует ${c} отрицательный/ых эффект/ов с основной цели, кроме ${excl}.` },
        { match: _P.bareDmgBonus, replace: (m, d) => `+${d} % урона.` },
        { match: _P.modeApplyPlusRandomAlly, replace: (m, mode, c, pr) => `${_modeLoc(mode, 'ru')}: применяет +${c} ${pr} на случайного союзника.` },
        { match: _P.dealAlliesHealth, replace: (m, p) => `Высасывает ${p} % от макс. здоровья союзников.` },
        ...
        { match: _P.targetHasDealRedistribute, replace: (m, proc, p, target) => `Если у основной цели есть ${proc}, высасывает ${p} % макс. здоровья цели и перераспределяет ${target}.` },
        ...
        { match: _P.selfHasLessThanGainMax, replace: (m, n, proc, c, pr, max) => `Если менее ${n} ${proc}, получает +${c} ${pr}, до максимума ${max}.` },
        ...
        { match: _P.modeSubTargetHasDeal, replace: (m, mode, sub, proc, p) => `${_modeLoc(mode, 'ru')}, ${sub}: если у основной цели есть ${proc}, высасывает ${p} % макс. здоровья цели.` },
        ...
        { match: _P.modeSubTargetNotTraitDeal, replace: (m, mode, sub, tr, p) => `${_modeLoc(mode, 'ru')}, ${sub}: если основная цель не ${tr}, высасывает ${p} % макс. здоровья цели.` },
        ...
        { match: _P.selfHasGain, replace: (m, proc, pr) => `Если есть ${proc}: получает ${pr}.` },
        { match: _P.selfHasLoseProc, replace: (m, proc, c, pr) => `Если есть ${proc}: теряет ${c} ${pr}.` },
        { match: _P.selfHasMoreSelfHasLoseProc, replace: (m, n, p1, p2, c, pr) => `Если ${p1} больше ${n}, Если есть ${p2}: теряет ${c} ${pr}.` },
        ...
        { match: _P.ifAllyApply, replace: (m, ally, pr) => `Если ${ally} союзник: применяет ${pr} к основной цели.` },
        ...
        { match: _P.otherwiseTargetHasGainPlus, replace: (m, proc, c, pr) => `Иначе, если у основной цели есть ${proc}: получает +${c} ${pr}.` },
        ...
        { match: _P.noteKilledCantRevive, replace: () => `Враги, убитые этой атакой, не могут быть воскрешены.` },
        { match: _P.noteAttackCantCountered, replace: () => `Эту атаку нельзя контратаковать.` },
        { match: _P.noteAttackCantBlocked, replace: () => `Эту атаку нельзя заблокировать.` },
        { match: _P.noteAttackCantDodged, replace: () => `От этой атаки нельзя уклониться.` },
        { match: _P.noteAttackCantMiss, replace: () => `Эта атака не может промахнуться.` },
        { match: _P.modeApplyProcAllies, replace: (m, mode, pr) => `${_modeLoc(mode, 'ru')}: применяет ${pr} к союзникам.` },
        ...
        { match: _P.selfHasOrLessGainMax, replace: (m, c, pr, c2, pr2, mx) => `Если у персонажа ${c} или менее ${pr}, получает +${c2} ${pr2}, до максимума ${mx}.` },
        ...
        // --- Batch 15 (final) ---
        { match: _P.targetHasRemoveTarget, replace: (m, pr1, pr2) => `Если основная цель имеет ${pr1}, удаляет ${pr2} с основной цели.` },
        { match: _P.targetHasBarrierAllies, replace: (m, pr, p) => `Если основная цель имеет ${pr}, барьер в ${p}% от макс. здоровья союзникам.` },
        { match: _P.otherwiseBarrierAllies, replace: (m, p) => `В противном случае, барьер в ${p}% от макс. здоровья союзникам.` },
        { match: _P.applyDurRandomTraitOrAllyExcl, replace: (m, pr, t, tr1, tr2) => `Применяет ${pr} на ${t} ходов к случайному союзнику ${tr1} или ${tr2} (кроме себя).` },
        { match: _P.energyNotFullApplyCountDurTarget, replace: (m, c, pr, t) => `Если энергия способности не полная, применяет +${c} ${pr} на ${t} ходов к основной цели.` },
        { match: _P.energyNotFullAttackPierce, replace: (m, p) => `Если энергия способности не полная, атакует с ${p}% пробиванием вместо этого.` },
        { match: _P.modeSubTargetHasRemoveTarget, replace: (m, mode, pr1, pr2) => `${_modeLoc(mode, 'ru')}: если основная цель имеет ${pr1}, удаляет ${pr2} с основной цели.` },
        { match: _P.notModeCountTarget, replace: (m, mode, c) => `${_notModeLoc(mode, 'ru')}: удаляет ${c} положительный(е) эффект(ы) с основной цели.` },
        { match: _P.modeSubNoteEnemiesKilled, replace: (m, mode) => `${_modeLoc(mode, 'ru')}: враги, убитые этой атакой, не могут быть воскрешены.` },
        { match: _P.selfHasAnyNegStealAllNeg, replace: () => `Если у персонажа есть отрицательные эффекты, крадёт все отрицательный(е) эффект(ы) у основной цели.` },
        { match: _P.otherwiseStealAllExcl, replace: (m, pr) => `В противном случае, крадёт все положительный(е) эффект(ы) у основной цели, кроме ${pr}.` },
        { match: _P.onAssistStealAllExcl, replace: (m, pr) => `При поддержке, крадёт все положительный(е) эффект(ы) у основной цели, кроме ${pr}.` },
        { match: _P.selfNoNegNotModeFlipTarget, replace: (m, mode, c) => `Если у персонажа нет отрицательных эффектов, ${_notModeLoc(mode, 'ru')}: переворачивает ${c} положительный(е) эффект(ы) в отрицательные на основной цели.` },
        { match: _P.otherwiseNotModeFlipAllies, replace: (m, mode, c) => `В противном случае, ${_notModeLoc(mode, 'ru')}: переворачивает ${c} отрицательный(е) эффект(ы) в положительные на союзниках.` },
        { match: _P.modeFlipNegAllies, replace: (m, mode, c) => `${_modeLoc(mode, 'ru')}: переворачивает ${c} отрицательный(е) эффект(ы) в положительные на союзниках.` },
        ...
        { match: _P.applyCountDurNInjured, replace: (m, c, pr, t, n) => `Применяет ${c} ${pr} на ${t} ходов к ${n} самым раненым союзникам.` },
        ...
        // All-allies targeting patterns (type=random, limit>=10)
        { match: _P.applyProcAllTraitAllies, replace: (m, pr, tr) => `Применяет ${pr} ко всем союзникам-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.applyProcDurAllTraitAllies, replace: (m, pr, t, tr) => `Применяет ${pr} на ${t} хода ко всем союзникам-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.applyMaxAllTraitAllies, replace: (m, c, pr, max, tr) => `Применяет +${c} ${pr} ко всем союзникам-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}, до максимума ${max}.` },
        { match: _P.healAllTraitAllies, replace: (m, tr, p) => `Исцеляет всех союзников-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)} на ${p} % от макс. здоровья.` },
        { match: _P.barrierAllTraitAllies, replace: (m, p, tr) => `Барьер в ${p} % от макс. здоровья всем союзникам-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
        { match: _P.flipNegToPosAllTraitAllies, replace: (m, c, tr) => `Переворачивает ${c} негативных эффектов в позитивные у всех союзников-${_traitLoc(tr, SENTENCE_TEMPLATES._activeDict)}.` },
